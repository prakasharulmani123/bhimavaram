...
/*			echo '<pre>';
			...
			for($i=0; $i<=$cpt; $i++)
			{
				...
				if($upload)
				{
					...
					$photolist.=$photodata['file_name'].',';
					$this->resize_image($photodata['file_name']);
				}
				
			}
			$photolist=rtrim($photolist,',');
			$data['pictures']=$photolist;
			unset($data['userfile']);
			$insert=$this->df->insert_data_id('areas',$data);
			if($insert)
			{
				$slug=url_title($insert.'-'.$data['name']);
				$this->df->update_record('areas',array('slug'=>$slug),array('id'=>$insert));
				set_message('success','Area added successfully!');
...
//			$data['sidebar']['custom']='jobs_add_sidebar';
			$data['footer']['js']=array('parsley.js','plugins/jquery.ui.widget.js','plugins/jquery.iframe-transport.js','plugins//jquery.fileupload.js','chained.js','edit_profile.js','picture_upload.js','bootstrap-datepicker.js','addarea.js');		
			...
			$data['content']['template']='admin/addarea.php';	
...
//			$data['content']['qualifications']=$this->df->get_multi_row('jobs_qualifications',false,false,false,array('name'=>'asc'));		
	...
	{
		$id=uridata(4);
		$delete=$this->db->simple_query("delete from areas where id='$id'");
		if($delete)
		{
			set_message('success','Area info deleted successfully!');
		}
		redirect('admin/areas/manage');
	}	
	
	function manage()
	{
		$data['content']['template']='admin/manage_areas';
		$data['footer']['js']=array('classy_admin.js');
		$offset=uridata(4) ? uridata(4) : 0;
		$limit=10;
		$cityid=userdata('cityid');
		$totallistings=$this->df->doquery("select id from areas");
		//fecthing result from db
		$total=count($totallistings);
		$this->load->library('pagination');					
		$config['base_url'] = base_url()."index.php/admin/areas/manage/";
		...
		$data['content']['areas']=$this->df->doquery("select * from areas order by id desc limit $offset,$limit");
		$this->layout->admin_publish($data);		
	}	
   ...
   function editarea()
   {
	   $id=uridata(4);
		...
		if($this->general->validateForm($data))
		{
			...
			$photolist=rtrim($photolist,',');
			if(strlen($photolist)>3)
			{
				$data['pictures']=$photolist;
			}
			unset($data['userfile']);
			$update=$this->df->update_record('areas',$data,array('id'=>$id));
			if($update)
			{
				set_message('success','Area info updated successfully!');
				redirect('admin/areas/manage');
			}
			else
			{
		...
		{		
			$data['header']['css']=array('jobs/add.css');
			$data['content']['area']=$this->df->get_single_row('areas',array('id'=>$id));
			$data['footer']['js']=array('parsley.js','plugins/jquery.ui.widget.js','plugins/jquery.iframe-transport.js','plugins//jquery.fileupload.js','chained.js','edit_profile.js','picture_upload.js','bootstrap-datepicker.js','addarea.js');		
			$data['content']['user']=$this->df->get_single_row('users',array('uid'=>userdata('uid')));
			$data['content']['types']=$this->df->get_multi_row('jobs_type',false,false,false,array('id'=>'asc'));
			$data['content']['template']='admin/editarea.php';	
			$this->layout->admin_publish($data);						
		}
   }	
