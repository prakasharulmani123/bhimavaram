...
<ul class="breadcrumb">
  ...
  <li><?php echo word_limiter($content['news']['title'], 8); ?></li>
</ul>
<div id="listing-details span13">
  <div class="widget-heading">
    <h1 class="noline"><?php echo $content['news']['title']; ?></h1>
  </div>
  <div class="ad-meta"> Posted on <?php echo date("d M, Y", strtotime($content['news']['date_added'])); ?> in <?php echo anchor('news/index/' . $this->df->get_field_value('news_categories', array('id' => $content['news']['category']), 'slug'), $this->df->get_field_value('news_categories', array('id' => $content['news']['category']), 'name')); ?> | <a href="#comments-box"><i class="icon-comment-alt"></i>&nbsp; <?php echo $content['news']['total_comments'] ?> Comments</a>
    <?php
        if (userdata('uid')) {
            ?>
...
//   
    ...
    <?php echo trim(htmlspecialchars_decode($content['news']['content'])); ?> </div>
</div>
<!--Listing-Meta-Right Ends-->
